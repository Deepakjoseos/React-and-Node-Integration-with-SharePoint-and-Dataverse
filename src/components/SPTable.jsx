...
const handleConfirmUpload = async () => {
  ...
  try {
    ...
    formData.append('FolderPath', sPData.SPfolderPath);
...
//Downloading sharePoint file code starts from here
  ...
  const handleDownloadPdf = async (row) => {
    if (row && row.ServerRelativeUrl) {
      try {  
        const response = await api.get(
          `/downloadfromSharepoint`,
          { params: { serverRelativeUrl: row.ServerRelativeUrl },
      ...
      link.href = url;
      link.setAttribute('download', row.ServerRelativeUrl.split('/').pop()); // Use the file name from the URL
...
const handleConfirmDelete = async () => {
  setOpenDeleteDialog(false);
  if (selectedRow && selectedRow.ServerRelativeUrl) {
    try {
      const response = await api.delete('/deleteFileFromSharePoint', {
        params: { serverRelativeUrl: selectedRow.ServerRelativeUrl, fileName: selectedRow.fileName }
      });
